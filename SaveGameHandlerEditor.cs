    ...
    public class SaveGameHandlerEditor : Editor
    {
        ...
        private void FindSaveables()
        {
            if (handler != null)
            {
                ...
                if(allMonoBehaviours.Any(x => x.type == null))
                {
                    foreach (var pair in allMonoBehaviours)
                    {
                        if (pair.type == null)
                            Debug.Log("Found null instance in: " + pair.go);
                    }
                }

                var saveablesQuery = from Pair in allMonoBehaviours
                                     let Instance = Pair.type
                                     where Instance != null
                ...
                var attributesQuery = from Pair in allMonoBehaviours
                                      let Instance = Pair.type
                                      where Instance != null
